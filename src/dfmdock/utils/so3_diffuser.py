...
class SO3Diffuser:
    ...
    def diffusion_coef(self, t):
        ...
        if self.schedule == 'logarithmic':
            ...
            g_t = self.sigma(t) * np.sqrt(2 * (np.log(self.max_sigma) - np.log(self.min_sigma))) 
        else:
            raise ValueError(f'Unrecognize schedule {self.schedule}')
        return g_t
    ...
    def torch_corrector(
        ...
        Returns:
            [..., 3] rotation vector at next step.
        """
        z = torch.randn(1, 3, device=grad.device)
        grad_norm = grad.norm()
        noise_norm = z.norm()
        step_size = (snr * noise_norm / grad_norm) ** 2 * 2
        perturb = step_size * grad
        if not is_last:
            perturb = perturb + torch.sqrt(step_size * 2) * z
