...
def get_info_from_pdb(pdb_path):
    ...
    pdb_file = pdb_file[~pdb_file.hetero]

    # Set of valid (chain_id, res_id) pairs for residues with complete backbone atoms
    valid_residues = set()

    # Loop through each unique chain and residue combination
    for chain_id in np.unique(pdb_file.chain_id):
        chain = pdb_file[pdb_file.chain_id == chain_id]
        
        for res_id in np.unique(chain.res_id):
            # Select atoms for the current residue within the chain
            residue_atoms = chain[chain.res_id == res_id]
            
            # Check if the residue contains all required backbone atoms
            if set(["N", "CA", "C"]).issubset(residue_atoms.atom_name.tolist()):
                # Store (chain_id, res_id) tuples for valid residues
                valid_residues.add((chain_id, res_id))

    # Create a mask that is True for atoms in valid residues within each chain
    mask = np.array([(chain_id, res_id) in valid_residues for chain_id, res_id in zip(pdb_file.chain_id, pdb_file.res_id)])

    # Apply the mask to get the filtered structure
    filtered_structure = pdb_file[mask]
    ...
    for chain_id in np.unique(filtered_structure.chain_id):
        ...
        aa_coords = chain_atoms.coord  # all-atom coordinates
        ...
        numbering, resn = struc.get_residues(chain_atoms)
        seq_list = [restype_3to1.get(three, "X") for three in resn]
        seq = ''.join(seq_list)

        info.append({"structure": chain_atoms, "seq":seq, "aa_coords":aa_coords, "bb_coords":bb_coords})
...
def main(args):
    ...
    # inference
    for id in id_list:
        info = get_info_from_pdb(f'{args.pdb_dir}/{id}.pdb')
        receptor = info[0]
        ligand = info[1]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pdb_dir", type=str, default=None, help="Path to the file or directory. If not provided, defaults to False.")
