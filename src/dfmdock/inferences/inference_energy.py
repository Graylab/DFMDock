...
from dfmdock.models.energy_model import Energy_Model
...
def run(config):
    ...
    # load model
    model = Energy_Model.load_from_checkpoint(
        config.ckpt, 
        map_location=device,
    )
    model.eval()
    ...
    for pdb_id in tqdm(pdb_id_list):
        ...
        for idx in range(num_samples):
            ...
            energy = model(batch).detach()

            metrics = {'id': pdb_id, 'index': idx, 're_energy': energy.item()}
