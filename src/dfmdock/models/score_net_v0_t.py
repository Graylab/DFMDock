...
class Score_Net(nn.Module):
    ...
    def forward(self, batch, predict=False, return_energy=False):
        ...
        node = node + self.single_t_embed(t)
        ...
        tr_score = tr_pred / (tr_norm + 1e-6) * self.tr_scale(torch.cat([tr_norm, t_embed], dim=-1))
        rot_norm = torch.linalg.vector_norm(rot_pred, keepdim=True)
        rot_score = rot_pred / (rot_norm + 1e-6) * self.rot_scale(torch.cat([rot_norm, t_embed], dim=-1))
