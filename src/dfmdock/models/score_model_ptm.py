...
from dfmdock.models.score_net_ptm import Score_Net
...
from dfmdock.utils.loss import get_tm_loss
...
class Score_Model(pl.LightningModule):
    ...
    def loss_fn(self, batch, eps=1e-5):
        ...
        # tm loss
        vec = batch["rec_pos"][:, None, 1, :] - batch["lig_pos"][None, :, 1, :]
        gt_vec = batch_gt["rec_pos"][:, None, 1, :] - batch_gt["lig_pos"][None, :, 1, :]
        sq_diff = torch.sum((vec - gt_vec) ** 2, dim=-1)
        tm_loss = get_tm_loss(logits, sq_diff) * 0.1

        # total losses
        loss = tr_loss + rot_loss + ec_loss + el_loss + ires_loss + tm_loss
        losses = {"tr_loss": tr_loss, "rot_loss": rot_loss, "ec_loss": ec_loss, "el_loss": el_loss, "ires_loss": ires_loss, "tm_loss": tm_loss, "loss": loss}
...
@hydra.main(version_base=None, config_path="/scratch4/jgray21/lchu11/graylab_repos/DFMDock/configs/model", config_name="score_model_ptm.yaml")
