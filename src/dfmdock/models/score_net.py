...
class EGNN_Force(nn.Module):
    def __init__(
        ...
        super(EGNN_Force, self).__init__()
        self.depth = depth
...
class EGNN_Energy(nn.Module):
    def __init__(
        ...
        super(EGNN_Energy, self).__init__()
        self.depth = depth
        for i in range(depth):
            self.add_module("EGNN_%d" % i, EGNNLayer(
                node_dim=node_dim, 
...
class Score_Net(nn.Module):
    ...
    def __init__(
        ...
        # energy network
        self.energy_network = EGNN_Energy(
            node_dim=node_dim, 
            edge_dim=edge_dim, 
            act_fn=nn.SiLU(), 
            depth=depth, 
    ...
    def forward(self, batch, predict=False, return_energy=False):
        ...
        # main network 
        node_out = self.energy_network(node, pos[..., 1, :], edge_index, edge_attr, lig_mask) # [R+L, H]
        pos_out = self.force_network(node, pos[..., 1, :], edge_index, edge_attr, lig_mask) # [R+L, H]
