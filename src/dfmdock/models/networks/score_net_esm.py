...
class Score_Net(nn.Module):
    ...
    def __init__(
        ...
        self.single_embed = nn.Linear(lm_embed_dim + inner_dim, node_dim, bias=False)
    ...
    def forward(self, batch, predict=False, return_energy=False):
        ...
        x = torch.cat([x, t_emb.repeat(x.size(0), 1)], dim=-1)
        ...
        edge_index = get_cross_graph_index(pos[..., 1, :], sep=rec_x.size(0), num_self=20, num_cross=40)

        # main network 
        node_out, tr_update, rot_update = self.network(node, pos[..., 1, :], edge_index) # [R+L, H]
