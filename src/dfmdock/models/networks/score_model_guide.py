...
class Score_Model(pl.LightningModule):
    ...
    def loss_fn(self, batch, eps=1e-5):
        ...
        with torch.no_grad():
            ...
            n = rec_pos.size(0)
            rec_ires = batch["ires"][:n]
            lig_ires = batch["ires"][n:]

            if random.random() < 0.5:
                rec_ires = sample_binary_tensor(rec_ires)
                lig_ires = sample_binary_tensor(lig_ires)
            else:
                rec_ires = torch.zeros_like(rec_ires)
                lig_ires = torch.zeros_like(lig_ires)

            batch["rec_x"] = torch.cat([batch["rec_x"], rec_ires], dim=-1)
            batch["lig_x"] = torch.cat([batch["lig_x"], lig_ires], dim=-1)
            batch_gt["rec_x"] = torch.cat([batch_gt["rec_x"], rec_ires], dim=-1)
            batch_gt["lig_x"] = torch.cat([batch_gt["lig_x"], lig_ires], dim=-1)
...
def randomly_mask_ones(tensor, n=4):
    ...
    # Select a number between 0 and the smaller of 4 or the total number of ones
    num_ones_to_keep = torch.randint(0, min(n, num_ones) + 1, (1,)).item()
...
def sample_binary_tensor(tensor, p=0.2):
    """
    Samples a binary tensor so that if p < 0.2, 1s remain 1, and if p > 0.2, 1s become 0.

    Parameters:
    tensor (torch.Tensor): Input binary tensor with values 0 and 1.
    p (float): Threshold probability for 1s to remain as 1.

    Returns:
    torch.Tensor: The modified tensor after applying the sampling condition.
    """
    # Create a mask of random probabilities
    random_probs = torch.rand_like(tensor, dtype=torch.float)

    # Apply the condition: keep 1 if random probability < p, otherwise set to 0
    result = torch.where((tensor == 1) & (random_probs < p), 1.0, 0.0)
...
@hydra.main(version_base=None, config_path="/scratch4/jgray21/lchu11/graylab_repos/DFMDock/configs/model", config_name="score_model_guide.yaml")
