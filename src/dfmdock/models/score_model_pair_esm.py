...
from dfmdock.models.score_net_pair_recycle import Score_Net
...
class Score_Model(pl.LightningModule):
    ...
    def encode(self, batch):
        node, edge = self.net.encode(batch)
        return node, edge

    def decode(self, batch):
        outputs = self.net.decode(batch)
        return outputs
    ...
    def loss_fn(self, batch, eps=1e-5):
        ...
        # dist loss
        dist_loss = distogram_loss(dist, pos_gt) 

        # total losses
        loss = tr_loss + rot_loss + ec_loss + el_loss + ires_loss + 0.1 * dist_loss
        losses = {"tr_loss": tr_loss, "rot_loss": rot_loss, "ec_loss": ec_loss, "el_loss": el_loss, "ires_loss": ires_loss, "dist_loss": dist_loss, "loss": loss}
    ...
    def step(self, batch, batch_idx):
        ...
        x = self.get_esm_rep(rec_seq + lig_seq)
...
@hydra.main(version_base=None, config_path="/scratch4/jgray21/lchu11/graylab_repos/DFMDock/configs/model", config_name="score_model_pair_esm.yaml")
