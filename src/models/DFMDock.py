...
class DFMDock(pl.LightningModule):
    def __init__(
        ...
        # confidence model
        self.use_tm_loss = experiment.use_tm_loss
    ...
    def loss_fn(self, batch, eps=1e-5):
        ...
        if self.use_tm_loss:
            ...
            noised_dist = torch.norm((batch["rec_pos"][:, None, 1, :] - batch["lig_pos"][None, :, 1, :]), dim=-1)
            sq_diff = (noised_dist - gt_dist) ** 2
            logits = outputs["tm_logits"]
            tm_loss = get_tm_loss(logits, sq_diff)
        else:
            tm_loss = torch.tensor(0.0, device=self.device)

        # total losses
        loss = tr_loss + rot_loss + ec_loss + el_loss + 0.1 * tm_loss
        losses = {"tr_loss": tr_loss, "rot_loss": rot_loss, "ec_loss": ec_loss, "el_loss": el_loss, "tm_loss": tm_loss, "loss": loss}
